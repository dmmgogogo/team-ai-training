...
| **DeepSeek V3.1** | 2025.8 | V3 系列迭代版（具体子版本/发布时间以官方发布与模型页为准） |
| **DeepSeek V3.1-Terminus** | 2025.9 | V3.1 系列后续迭代（以官方发布与模型页为准） |
| **DeepSeek V3.2-Exp** | 2025.9 | 实验版/探索版（以官方发布与模型页为准） |
| **DeepSeek V4 Lite** | 2026.3 | 迭代版（社区常见称呼；规格/是否开源/是否多模态以官方为准） |
| **DeepSeek V4** | 未确认 | 如出现“即将发布/预计某月发布”等信息，建议仅在引用官方公告时写入 |

> 📌 **信息来源建议优先级：** 官方公告/官方 API 文档与价格页 → 官方模型页面（如 Hugging Face）→ 第三方媒体/百科（仅作参考）

**为什么 DeepSeek 引爆全球？**
- DeepSeek V3 训练成本仅 **557 万美元**，而 GPT-4 约 1 亿美元
- 性能评测接近 GPT-4o，且 API 定价长期以“性价比”著称（具体以官方价格页为准）
- **R1 完全开源**，可本地部署，不受出口限制
- V3.1 在 SWE-bench（代码评测）和 Terminal-bench 上大幅超越前代

**API 价格：**
- 大模型价格波动很快（含促销/缓存/峰谷等策略差异），建议在课程中只保留“相对更便宜/更贵”的结论，并在文末附上官方价格入口，避免具体数字快速过期。
