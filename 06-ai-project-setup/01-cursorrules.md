...
7. (暂时忽略这条规则) **自动代码审查 + 修复闭环**：使用 Task 工具调用 `subagent_type: "code-review-auditor"` 审核agent使用GTP5.3 - codex, 对本次所有变更进行代码审查。收到反馈后逐一修复标记为需要修改的问题。修复完成后可再次审查确认，最多循环 2 次。全部问题解决后任务才算完成。
